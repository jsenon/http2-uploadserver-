...
build_local:
	go mod tidy
	go build -ldflags '${LDFLAGS}' -o http2-uploadserver  
build:
	go mod tidy
	docker build $(DOCKER_BUILD_ARGS) -t $(DOCKER_USER)/http2-uploadserver:$(APP_VERSION) -f ./build/Dockerfile .
lint:
ifndef GOLINTER
...
artifact_linux:
	CGO_ENABLED=0 GOOS=linux go build -ldflags '${LDFLAGS}' -o http2-uploadserver-linux

artifact_darwin:
	CGO_ENABLED=0 GOOS=darwin go build -ldflags '${LDFLAGS}' -o http2-uploadserver-darwin
...
publish: 
	docker push $(DOCKER_USER)/http2-uploadserver:$(APP_VERSION)
